...
def ecu_mod_recipe():
    ...
    print(
        "Clearing out old patch data to allow for this to be used for development."
    )
    ...
    print(
        "======Running CMake to get environment variables setup to build.======"
    )

    cmake_process = subprocess.Popen([
        "cmake", "-D", "CMAKE_EXPORT_COMPILE_COMMANDS:BOOL=TRUE", "-D",
        "CMAKE_C_COMPILER=tricore-gcc.exe", "-S", ".", "-B", "./build", "-G",
        "Unix Makefiles"
    ],
    ...
    build_process = subprocess.Popen(["cmake", "--build", "."],
                                     cwd="build",
                                     stdout=subprocess.PIPE,
                                     stderr=subprocess.STDOUT)
    ...
    ida_elftools_patchhelper.import_elf(elf_file, code_segments,
                                        rodata_segments)

    print("======Finding where to patch and patching======")
    get_map_value_calls = ida_find_function_calls.find_function_calls(
        "get_map_value", "calcFuel")
